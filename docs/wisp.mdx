...
Prior to the creation of Wisp, proxy traffic over backends such as the Bare Server or Rammerhead was sent completely unencrypted to the server host. This may work fine if you are hosting your own proxy, but when using public links obtained from a Discord server, the risk of the host stealing your tokens from proxy usage might be a problem. 
...
- [Ultraviolet](https://github.com/titaniumnetwork-dev/Ultraviolet/) - A sophisticated web proxy using libcurl.js or epoxy. 
...
#### Behavior
The client must send a CONNECT packet to the server to create a new stream under the same web socket. The stream ID chosen by the client at this point will be associated with this stream for all future messages. When the server receives this packet, it must validate this information, and if the payload is invalid, a CLOSE packet must be sent.

Once the payload has been validated, the server must immediately try to establish a TCP/UDP socket to the specified hostname and port. If this fails, the server must send a CLOSE packet with the reason. To reduce overall delay, the client can begin sending data before the server receives a CONTINUE packet.
...
| Field Name     | Field Type | Notes                                                      |
|----------------|------------|------------------------------------------------------------|
| Stream Payload | `char[]`   | The data that is sent to and from the destination server. |

#### Behavior
Any DATA packets sent from the client to the server must be proxied to the TCP/UDP socket associated with the packet's stream ID. On the server, the received payload must be buffered before being sent to the TCP/UDP socket to handle congestion. The size of this send buffer is predetermined and must be the same for every stream. 

Any DATA packets sent from the server to the client must be interpreted as coming from the TCP/UDP socket associated with the packet's stream ID.

For TCP streams, the server must buffer any packets received from the client in a FIFO queue and keep a separate buffer for each TCP stream.
...
The server must send another CONTINUE packet when it receives the same number of packets from the client as its maximum buffer size. The server should regularly send CONTINUE packets before this point to ensure minimal delays when receiving data from the client. 
...
#### Behavior
Any CLOSE packets sent from either the server or the client must immediately close the associated stream and TCP socket. The close reason in the payload doesn't affect this behavior but may provide extra information useful for debugging.

#### Client/Server Close Reasons
- `0x01` - Reason unspecified or unknown. Returning a more specific reason should be preferred.
- `0x02` - Voluntary stream closure, equating to one side resetting the connection.
...
- `0x42` — Stream creation failed due to an unreachable destination host. This could be sent if the destination is a domain that does not resolve to anything. 
- `0x43` - Stream creation timed out due to the destination server not responding.
- `0x44` - Stream creation failed because the destination server refused the connection.
- `0x47` - TCP data transfer timed out.
- `0x48` - The proxy server intentionally blocks the stream destination address/domain.
- `0x49` - Connection throttled by the server.

#### Client Only Close Reasons
- `0x81` - The client has encountered an unexpected error and cannot receive any more data. 

## HTTP Behavior
Since the entire protocol takes place over WebSockets, a few rules must be in place to ensure an HTTP connection can be upgraded successfully.

### Server Architecture
The server must have an HTTP and WebSocket server that conforms to the respective standards.
...
The server implementation interprets the prefix. It may be ignored or used for gatekeeping to establish password-based authentication.
...
Immediately after a WebSocket connection is established, the server must send a CONTINUE packet containing the initial buffer size for each stream. The stream ID for this packet must be set to 0, corresponding to the Wisp protocol version (0 means Wisp v1). The client must wait for this CONTINUE packet to be received before beginning any communications. The purpose of this packet is to ensure that the client does not have to wait for a CONTINUE packet to create each stream, reducing the overall delay.
