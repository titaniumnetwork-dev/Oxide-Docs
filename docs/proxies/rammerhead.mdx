...
```
git clone https://raw.githubusercontent.com/binary-person/rammerhead
cd rammerhead
npm install
npm run build
...
Configure your settings in [src/config.js](src/config.js). If you wish to consistently pull updates from this repo without the hassle of merging, create `config.js` in the root folder so they override the configs in `src/`.
