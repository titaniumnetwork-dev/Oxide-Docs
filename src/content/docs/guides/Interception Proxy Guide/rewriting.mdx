...
This is commonly accoplished by wrapping all member expressions' properties in a wrapper function, which is fine initially. To allow more features (and potentially improve Google Botguard score), Precise DPSC passes the object being accessed into the property rewriter as well, allowing more targeted propty rewriting.
...
This method is currently not used in TitaniumNetwork proxies, but can be found in generic rewriters such as [rewd](https://github.com/portal-co/rewd).
