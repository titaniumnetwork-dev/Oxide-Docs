...
It's not recommended to use interception for Request URLs. The methods are either limited, or they expose the real URLs to the filtering extensions
...
This refers to setting a base URL to be under the route that the SW handles under its prefix. This is only useful for relative URLs. There's no point of doing this, because you also need to handle absolute URLs.
