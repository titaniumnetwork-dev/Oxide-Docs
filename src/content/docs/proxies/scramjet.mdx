...
Until Scramjet has an NPM package, you have to use the CI builds, which run every time a commit is pushed to `main`.
If you want to upgrade to a new CI build and you use PNPM, you must type `pnpm cache delete @mercuryworkshop/scramjet` and then install it again.
